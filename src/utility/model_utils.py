...
def compute_abd_cov(X, labels_components, class_dict, component_dict=None, batch_idx=-1, total_progress=-1):
    ...
    if component_dict:
        ec_mapped = np.zeros(shape=(X.shape[0], len(class_dict.keys()), len(component_dict.keys())))
    ...
    for class_idx, class_label in class_dict.items():
        ...
        if total_progress > 0:
            ...
            desc = '\t\t--> Building {0:.4f}%...'.format(((current_progress / total_progress) * 100))
            print(desc, end="\r")
    if component_dict:
...
def __synthesize_report(X, sample_ids, y_pred_score, y_pred, y_dict_ids, y_common_name, component_dict,
    ...
    desc = '\t\t--> Synthesizing pathway reports {0:.4f}%...'.format(((batch_idx + 1) / total_progress * 100))
    print(desc, end="\r")
    ...
    for sidx, item in enumerate(y_pred):
        ...
        if y_pred_score is not None:
            sample_data = np.c_[lst_ptwy, lst_ptwy_name, label_prob, label_prediction, label_abun, label_cov, ec2label]
        else:
            sample_data = np.c_[lst_ptwy, lst_ptwy_name, label_prediction, label_abun, label_cov, ec2label]
        ...
        if y_pred_score is not None:
            df_sample_pathways.columns = ['FrameID', 'Name', 'Score', 'Predicted', 'Abundance', 'Coverage', 'MappedEC']
        else:
            df_sample_pathways.columns = ['FrameID', 'Name', 'Predicted', 'Abundance', 'Coverage', 'MappedEC']
        df_sample_pathways.to_csv(path_or_buf=os.path.join(sample_folder_path, file_detail_name), sep='\t')
...
def psndcg(y_prob, y_true, A=1, B=1, C=1, top_k=50):
    ...
    psdcg = N_j / psp_label
    labels_idx = np.flip(np.argsort(y_prob))[:, :top_k]
    tmp = np.array([psdcg[s_idx, labels_idx[s_idx]] for s_idx in np.arange(psdcg.shape[0])])
